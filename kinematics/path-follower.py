...
#        total_angle(p7-p6,p5-p6,[0,0,1])-q3*pi/180
        x11-x,
        y11-y,
        total_angle(p12-p13,numpy.array((1,0)),[0,0,1])-q,
        ]
...
ini = [-56.00000073,  62.99999982, 
       -60.54639098,  37.21778389,
       -44.30234921,  20.93493667, 
       -25.65321163,  28.16056938,
        -6.00000192,  62.99950351,  
        47.99730506,  63.53861903,
        68.34204674,  29.09894105,  
        21.57794614,   5.23748706,
        64.51609652, -15.07421387,  
        45.8675778 , -22.30144384,
       -10.29191843, -43.42278492,   
       8.6414348 , -49.86701318,
...
lwpolys = foldable_robotics.dxf.read_lwpolylines(filename)
...
poly*=(.6,1)
poly+=a
v = numpy.r_[poly[1:],poly[0:1]]-poly
l = ((v**2).sum(1))**.5
lc = l.cumsum()
l_total = l.sum()
lengths = numpy.r_[0:l_total:((num_segments+1)*1j)][:-1]
#    print(lengths)
...
for item in lengths:
    ...
    if ii==0:
        ...
        m = (item-lc[ii-1])/(lc[ii]-lc[ii-1])
    print(m)
    v.append((poly[ii]*(1-m)+poly[ii+1]*m))
v = numpy.array(v)

plt.plot(*(poly.T))
plt.plot(*(v.T),'ro')
        
#    for p0,p1 in zip(poly,numpy.r_[poly[1:],poly[0:1]]):
#        v 


#plt.figure()
result = scipy.optimize.minimize(generate_solver(v[0,0],v[0,1],0),ini)
plotresult(result.x)
plt.axis('equal')
...
for x,y in v:
    ...
    result = scipy.optimize.minimize(generate_solver(x,y,0),ini1)
